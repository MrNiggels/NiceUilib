...
ui = {} do
 ...
 local window_scrolldown = Instance.new("TextButton")
 window_scrolldown.Text = "↓"
 window_scrolldown.ClipsDescendants = true
 window_scrolldown.AutoButtonColor = false
 window_scrolldown.BackgroundColor3 = ui.colors.button
 window_scrolldown.BackgroundTransparency = 1
 window_scrolldown.BorderSizePixel = 0
 window_scrolldown.TextColor3 = ui.colors.text
 window_scrolldown.Font = ui.Font
 window_scrolldown.TextSize = ui.FontSize+4
 window_scrolldown.Size = UDim2.new(0, 30-4, 0, 30-4)
 window_scrolldown.Position = UDim2.new(1, -90+2, 0, 2)
 window_scrolldown.ZIndex = 37
 window_scrolldown.Parent = window_clip2
 round(window_scrolldown)
 
 window_scrolldown.MouseEnter:Connect(function() 
 twn(window_scrolldown, {BackgroundTransparency = 0.4})
 end)
 
 window_scrolldown.MouseLeave:Connect(function() 
 twn(window_scrolldown, {BackgroundTransparency = 1})
 end)



 local window_scrollup = Instance.new("TextButton")
 window_scrollup.Text = "↑"
 window_scrollup.ClipsDescendants = true
 window_scrollup.AutoButtonColor = false
 window_scrollup.BackgroundColor3 = ui.colors.button
 window_scrollup.BackgroundTransparency = 1
 window_scrollup.BorderSizePixel = 0
 window_scrollup.TextColor3 = ui.colors.text
 window_scrollup.Font = ui.Font
 window_scrollup.TextSize = ui.FontSize+4
 window_scrollup.Size = UDim2.new(0, 30-4, 0, 30-4)
 window_scrollup.Position = UDim2.new(1, -120+2, 0, 2)
 window_scrollup.ZIndex = 35
 window_scrollup.Parent = window_clip2
 round(window_scrollup)
 
 window_scrollup.MouseEnter:Connect(function() 
 twn(window_scrollup, {BackgroundTransparency = 0.4})
 end)
 
 window_scrollup.MouseLeave:Connect(function() 
 twn(window_scrollup, {BackgroundTransparency = 1})
 end)
 ...
 window_scrolldown.Parent = window_topbar
 window_scrollup.Parent = window_topbar
 ...
 if old_input.UserInputType == Enum.UserInputType.MouseButton1 or old_input.UserInputType == Enum.UserInputType.Touch then
 ...
 if cur_input.UserInputType == Enum.UserInputType.MouseButton1 or cur_input.UserInputType == Enum.UserInputType.Touch then
 pcall(function() 
 ...
 window_scrolldown.MouseButton1Click:Connect(function()
  local newpos = menu_menu.CanvasPosition.Y + (ui.ScrollAmount/2)
  local max = menu_menu.AbsoluteCanvasSize.Y - menu_menu.AbsoluteSize.Y
 ...
 window_scrollup.MouseButton1Click:Connect(function()
  local newpos = menu_menu.CanvasPosition.Y - (ui.ScrollAmount/2)
  local max = menu_menu.AbsoluteCanvasSize.Y - menu_menu.AbsoluteSize.Y
 ...
 local oldScroll = menu_scroll.Visible
 local oldSize = menu_menu.CanvasSize
 local function open() 
  
  oldScroll = menu_scroll.Visible
  oldSize = menu_menu.CanvasSize
  local amount = #options
  menu_menu.CanvasSize = menu_menu.CanvasSize + UDim2.new(0, 0, 0, 28*amount)
 ...
 menu_menu.CanvasSize = oldSize
 menu_scroll.Visible = oldScroll 
 OnClose:Fire()
 OnToggle:Fire(false)
 end
  ...
  for i, v in pairs(options) do
    ...
    dropdown_option.MouseButton1Click:Connect(function()
      ...
      for i, v in pairs(options_buttons) do
        twn(v, {
          BackgroundColor3 = ui.colors.button,
          BackgroundTransparency = 0.7
        })
      end
      twn(dropdown_option, {
        BackgroundColor3 = ui.colors.enabled,
        BackgroundTransparency = 0.4
      })
      selection = {
        v,
 ...
 function t:SetOptions(t)
  for i,v in next, options_buttons do
    v:Destroy()
  end
  task.wait()
  for i,v in next, options_buttons do
    table.remove(options_buttons, i)
  end
  task.wait(0.1)
  create(t)
  if open_state then open() else close() end
 end
